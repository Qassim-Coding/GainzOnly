...
function App() {
  ...
  const openStockWidget = (input) => {
    const isObj = input && typeof input === "object";
    const symbol = isObj ? String(input.symbol || "").toUpperCase() : String(input || "").toUpperCase();
    const exchange = isObj ? input.exchange : undefined;

    const found = stocks.find((x) => x.symbol.toUpperCase() === symbol);

    const tvSymbol = exchange
      ? `${exchange}:${symbol}`
      : found?.exchange
    ...
    <div className="min-h-dvh p-4 mx-auto flex flex-col xl:flex-row gap-5 text-white bg-slate-900">
      <header className="flex flex-col gap-4 xl:w-1/2">
        <div className="text-center">
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold leading-tight mx-auto max-w-4xl">
        ...
        <div className="flex flex-nowrap overflow-x-auto whitespace-nowrap gap-2 justify-start md:justify-center" role="tablist" aria-label="Navigation des sections">
          <button
            onClick={() => setActiveTab("portfolio")}
            role="tab"
            aria-selected={activeTab === "portfolio"}
            className={`relative shrink-0 px-3 py-2 rounded-lg border transition-colors outline-none focus-visible:ring-2 focus-visible:ring-teal-400/60 ${
              activeTab === "portfolio"
        ...
        <div className="relative w-full rounded-md mb-2 bg-gray-100">
          <div className="block w-full pb-[56.25%] md:pb-[50%] xl:pb-[40%]" />
          <div
            ref={stockWidgetRef}
            className="tradingview-widget-container absolute inset-0 rounded-md"
          />
        </div>
